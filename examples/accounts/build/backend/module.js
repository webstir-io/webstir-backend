...
import { createModule, fromTsRestRouter } from "@webstir-io/module-contract";
import { z } from "zod";
...
const routeSpecs = fromTsRestRouter({
  router,
  baseName: "accounts",
  createRoute: ({ keyPath }) => ({
    handler: async (ctx) => {
      if (keyPath.at(-1) === "detail") {
        ...
        return row ? { status: 200, body: row } : { status: 404, errors: [{ code: "not_found", message: "Account not found" }] };
      }
